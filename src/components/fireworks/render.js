...
export const Render = {
  ...
  _init: function () {
    this._node = document.getElementById("fireworks");
    var o = {
      alpha: false,
    };
    this.gl =
      this._node.getContext("webgl", o) ||
      this._node.getContext("experimental-webgl", o);
    if (!this.gl) {
      alert(
        "Sorry, no WebGL API support detected. Get a modern browser and come back again."
      );
      return;
    }
  ...
  _sync: function () {
    ...
    if (w != this.gl.drawingBufferWidth || h != this.gl.drawingBufferHeight) {
      alert(
        "Your WebGL implementation seems to be having troubles with its drawing buffer size."
      );
    }
  },
...
window.addEventListener("resize", () => Render._sync());
